...
type View =
  ...
  | { type: 'add-player' }
  | { type: 'invite-form'; player: Player }
  | { type: 'success'; player: Player }
...
export default function KaderListe({
  ...
  function handleInvite(e: React.FormEvent<HTMLFormElement>, player: Player) {
    e.preventDefault()
    const formData = new FormData(e.currentTarget)
    setFormError('')
    startTransition(async () => {
      try {
        await inviteContact(player.id, formData)
        setView({ type: 'success', player })
      } catch (err: unknown) {
  ...
  function renderHeader() {
    if (view.type === 'list') {
      return (
        <div className="bg-[#9B1C2E] px-4 pt-8 pb-5 flex items-end justify-between">
          ...
          <button
            onClick={() => { setFormError(''); setView({ type: 'add-player' }) }}
    ...
    if (view.type === 'invite-form') {
      return (
        <div className="bg-[#9B1C2E] px-4 pt-8 pb-5">
          <div className="flex items-center gap-3">
            <button
              onClick={() => { setFormError(''); setView({ type: 'player', player: view.player }) }}
              className="w-8 h-8 rounded-lg bg-white/15 flex items-center justify-center flex-shrink-0"
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  function renderContent() {
    ...
    if (view.type === 'add-player') {
      return (
        <form onSubmit={handleAddPlayer} className="px-3 pt-4 pb-20 flex flex-col gap-3">
    ...
    if (view.type === 'player') {
      const playerContacts = contactsByPlayer.get(view.player.id) ?? []
      return (
        <div className="px-3 pt-3 pb-20 flex flex-col gap-2">
          ...
          <button
            onClick={() => { setFormError(''); setView({ type: 'invite-form', player: view.player }) }}
    ...
    if (view.type === 'invite-form') {
      return (
        <form
          onSubmit={e => handleInvite(e, view.player)}
          className="px-3 pt-4 pb-20 flex flex-col gap-3"
        >
          <div className="bg-white rounded-xl px-4 py-3 border border-gray-100">
          ...
          <div className="flex gap-2.5 items-start bg-blue-50 rounded-xl px-4 py-3 border border-blue-100">
            ...
            <p className="text-xs text-blue-700 leading-relaxed">
              {view.player.first_name} wird mit diesem Kontakt verknüpft. Die eingeladene Person erhält
              eine E-Mail und kann die App für {view.player.first_name} nutzen.
            </p>
          </div>
        ...
        <div className="flex flex-col items-center justify-center px-6 py-16 gap-4">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500 leading-relaxed">
              Der Kontakt erhält eine E-Mail und wird dann mit {view.player.first_name} verknüpft.
            </p>
          </div>
          <button
            onClick={() => setView({ type: 'player', player: view.player })}
